...
function M.mark_hunk_addressed(worktree_path, filepath, hunk, action)
	...
	table.insert(
		...
		{ filepath = filepath, hunk_hash = M.hunk_hash(hunk), action = action, timestamp = os.time() }
	)
...
function M.is_file_fully_addressed(worktree_path, filepath)
	...
	for _, hunk in ipairs(hunks) do
		if addressed_hashes[M.hunk_hash(hunk)] then
...
function M.accept_file_from_worktree(worktree_path, filepath, repo_root)
	repo_root = repo_root
		or (M.worktrees[worktree_path] and M.worktrees[worktree_path].repo_root)
		or M.get_repo_root(worktree_path)
	if not repo_root then
...
function M.accept_hunk_from_worktree(worktree_path, filepath, hunk, user_file_path)
	...
	if #hunk.user_added_indices == #hunk.removed_lines and #hunk.added_lines == 0 then
		return true, nil
	end
			...
			if start == 0 then
				...
				for _, idx in ipairs(hunk.user_added_indices or {}) do
					if hunk.removed_lines[idx] then
						table.insert(lines, hunk.removed_lines[idx])
					end
				end
			else
				for i, l in ipairs(hunk.added_lines) do
...
function M.is_session_active()
	return M.vibe_branch ~= nil and M.snapshot_commit ~= nil
end
