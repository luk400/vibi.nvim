...
local function build_popup_content(hunk)
	...
	if #user_adds > 0 then
		for _, line in ipairs(user_adds) do
			...
			if #display_line > conf.width - 2 then
				display_line = display_line:sub(1, conf.width - 5) .. "..."
			end
...
function M.accept_user()
	apply_resolution(git.reject_hunk_from_worktree, "user", "Accepted your version")
end
function M.accept_ai()
	apply_resolution(git.accept_hunk_from_worktree, "ai", "Accepted AI's version")
end
function M.accept_both()
	apply_resolution(git.keep_both_hunk, "both", "Kept both versions")
end
function M.accept_none()
	apply_resolution(git.delete_hunk_range, "none", "Removed all changes")
end

function M.setup_keymaps()
	if not M.popup_bufnr then
		return
	end
