...
function M.render(bufnr)
	...
	for hunk_idx, hunk in ipairs(hunks) do
		...
		if M.has_overlap(hunk) then
			...
			local conflict_text =
				string.format("<<<<<<< CONFLICT: %d lines affected (cursor here to resolve) >>>>>>>", total_lines)
	...
	if git.is_file_fully_addressed(worktree_path, filepath) then
		...
		if new_hunks and #new_hunks > 0 then
			...
			for _, h in ipairs(new_hunks) do
				if not addressed_hashes[git.hunk_hash(h)] then
					table.insert(unaddressed_hunks, h)
				end
			end

			if #unaddressed_hunks > 0 then
				unaddressed_hunks =
					M.classify_user_additions(unaddressed_hunks, worktree_path, filepath, user_file_path)
...
function M.accept_hunk()
	process_hunk_action(git.accept_hunk_from_worktree, "accepted", "Hunk accepted")
end
function M.reject_hunk()
	process_hunk_action(git.reject_hunk_from_worktree, "rejected", "Hunk rejected")
end
...
function M.accept_all_in_file()
	...
	for _, hunk in ipairs(hunks) do
		...
		if ok then
			git.mark_hunk_addressed(worktree_path, filepath, hunk, "accepted")
		end
	end
...
function M.setup_keymaps(bufnr)
	...
	if keymaps.accept_hunk then
		vim.keymap.set("n", keymaps.accept_hunk, M.accept_hunk, vim.tbl_extend("force", opts, { desc = "Accept hunk" }))
	end
	if keymaps.reject_hunk then
		vim.keymap.set("n", keymaps.reject_hunk, M.reject_hunk, vim.tbl_extend("force", opts, { desc = "Reject hunk" }))
	end
	if keymaps.accept_all then
		vim.keymap.set(
			"n",
			keymaps.accept_all,
			M.accept_all_in_file,
			vim.tbl_extend("force", opts, { desc = "Accept all in file" })
		)
	end
	if keymaps.reject_all then
		vim.keymap.set(
			"n",
			keymaps.reject_all,
			M.reject_all_in_file,
			vim.tbl_extend("force", opts, { desc = "Reject all in file" })
		)
	end
	if keymaps.prev_hunk then
		vim.keymap.set("n", keymaps.prev_hunk, M.prev_hunk, vim.tbl_extend("force", opts, { desc = "Previous hunk" }))
	end
	if keymaps.next_hunk then
		vim.keymap.set("n", keymaps.next_hunk, M.next_hunk, vim.tbl_extend("force", opts, { desc = "Next hunk" }))
	end
	...
	if config.options.diff.conflict_popup.enabled then
		M.setup_conflict_tracking(bufnr)
	end
end
	...
	vim.api.nvim_create_autocmd("CursorMoved", {
		...
		callback = function()
			...
			if hunk and M.has_overlap(hunk) then
				if not conflict_popup.is_visible() then
					conflict_popup.show(bufnr, hunk, idx)
				end
			end
		end,
	})
