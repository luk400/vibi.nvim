...
local function parse_conflict_markers(lines)
  ...
  while i <= #lines do
    if lines[i]:match("^<<<<<<< HEAD") then
      ...
      for j = i + 1, #lines do
        if lines[j]:match("^=======") then separator_line = j - 1; break end
      end
      if separator_line then
        for j = separator_line + 2, #lines do
          if lines[j]:match("^>>>>>>> vibe%-") then conflict_end = j - 1; break end
        end
      end
      if separator_line and conflict_end then
        ...
        if ours_start > ours_end then ours_start, ours_end = 0, -1 end
        if theirs_start > theirs_end then theirs_start, theirs_end = 0, -1 end

        -- Create a dummy hunk for fallback compatibility with git.lua
        local rem_lines, add_lines = {}, {}
        for k = ours_start + 1, ours_end + 1 do table.insert(rem_lines, lines[k]) end
        for k = theirs_start + 1, theirs_end + 1 do table.insert(add_lines, lines[k]) end
        
        local dummy_hunk = {
          old_start = ours_start, old_count = #rem_lines,
          new_start = theirs_start, new_count = #add_lines,
          removed_lines = rem_lines, added_lines = add_lines,
          type = "change"
        }

        table.insert(conflicts, {
          start_line = conflict_start, end_line = conflict_end,
          ours_start = ours_start, ours_end = ours_end,
          theirs_start = theirs_start, theirs_end = theirs_end,
          resolved = false,
          hunk = dummy_hunk
        })
        i = conflict_end + 2
...
function M.insert_conflict_markers(user_lines, worktree_path, filepath, session_name, review_mode)
  ...
  local cmd = string.format("git merge-file -p -q -L HEAD -L Base -L vibe-%s %s %s %s", 
      ...
      if review_mode == "auto" then
        ...
        if count_b > 0 then
          table.insert(auto_merged_regions, {
            type = "add", start_line = highlight_start - 1, end_line = highlight_end - 1, hunk = dummy
          })
        elseif count_a > 0 and count_b == 0 then
          table.insert(auto_merged_regions, {
            type = "delete", start_line = highlight_start - 1, end_line = highlight_start - 1, hunk = dummy
          })
        end
        curr_a = a_last + 1
      else
        ...
        for k = a_first, a_last do
          if user_lines[k] then table.insert(final_lines, user_lines[k]) end
        end
        table.insert(final_lines, "=======")
        for k = b_first, b_last do
          if merged_output[k] then table.insert(final_lines, merged_output[k]) end
          curr_b = curr_b + 1
        end
...
function M.show_file_with_conflicts(worktree_path, filepath, _, review_mode)
  ...
  local lines_with_markers, conflicts, auto_merged_regions = M.insert_conflict_markers(user_lines, worktree_path, filepath, info.name, review_mode)
  ...
  if #conflicts > 0 then 
    ...
    vim.notify(string.format(
      ...
      #conflicts, keymaps.keep_ours or "o", keymaps.keep_theirs or "t",
      keymaps.keep_both or "b", keymaps.keep_none or "n"
    ), vim.log.levels.INFO)
  else
...
function M.setup_highlights(bufnr)
  ...
  for _, conflict in ipairs(state.conflicts) do
    if not conflict.resolved then
      ...
      if conflict.ours_start > conflict.ours_end then separator_line = conflict.start_line + 1 end
...
function M.resolve_conflict(resolution)
  ...
  if conflict.ours_start <= conflict.ours_end then
    for i = conflict.ours_start, conflict.ours_end do
      if lines[i + 1] then table.insert(ours_lines, lines[i + 1]) end
    end
  end
  if conflict.theirs_start <= conflict.theirs_end then
    for i = conflict.theirs_start, conflict.theirs_end do
      if lines[i + 1] then table.insert(theirs_lines, lines[i + 1]) end
    end
  end
  ...
  if resolution == "ours" then action = "rejected"
  elseif resolution == "theirs" then action = "accepted" end
  ...
  else vim.notify(string.format("[Vibe] Conflict resolved. %d remaining", remaining), vim.log.levels.INFO) end
...
function M.count_remaining_conflicts()
  ...
  local count = 0
  for _, c in ipairs(state.conflicts) do if not c.resolved then count = count + 1 end end
  return count
end
...
function M.next_conflict()
  ...
  for _, c in ipairs(state.conflicts) do
    if not c.resolved and c.start_line > cursor_line then
      vim.api.nvim_win_set_cursor(0, { c.start_line + 1, 0 }); return
    end
  end
  for _, c in ipairs(state.conflicts) do
  ...
  for i = #state.conflicts, 1, -1 do
    ...
    if not conflict.resolved then
      ...
      for line_idx = start_idx, end_idx do
        if lines[line_idx + 1] then table.insert(r_lines, lines[line_idx + 1]) end
      end
      ...
      if conflict.hunk then git.mark_hunk_addressed(state.worktree_path, state.filepath, conflict.hunk, resolution == "ours" and "rejected" or "accepted") end
...
function M.accept_all() process_all_conflicts("theirs") end
function M.reject_all() process_all_conflicts("ours") end
...
function M.quit()
  ...
  if worktree_path then require("vibe.dialog").show(worktree_path) end
end
...
function M.setup_keymaps(bufnr)
  ...
  if keymaps.keep_ours then vim.keymap.set("n", keymaps.keep_ours, M.keep_ours, opts) end
  if keymaps.keep_theirs then vim.keymap.set("n", keymaps.keep_theirs, M.keep_theirs, opts) end
  if keymaps.keep_both then vim.keymap.set("n", keymaps.keep_both, M.keep_both, opts) end
  if keymaps.keep_none then vim.keymap.set("n", keymaps.keep_none, M.keep_none, opts) end
  if keymaps.next_conflict then vim.keymap.set("n", keymaps.next_conflict, M.next_conflict, opts) end
  if keymaps.prev_conflict then vim.keymap.set("n", keymaps.prev_conflict, M.prev_conflict, opts) end
  if keymaps.accept_all then vim.keymap.set("n", keymaps.accept_all, M.accept_all, opts) end
  if keymaps.reject_all then vim.keymap.set("n", keymaps.reject_all, M.reject_all, opts) end
  if keymaps.quit then vim.keymap.set("n", keymaps.quit, M.quit, opts) end
