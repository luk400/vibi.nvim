...
local function get_current_line(bufnr, idx)
  ...
  if marks and #marks > 0 then return marks[1] end
  return nil
end
...
function M.show_file_with_collapsed_conflicts(worktree_path, filepath, _, review_mode)
  ...
  local lines_with_markers, conflicts, auto_merged_regions = conflict_buffer.insert_conflict_markers(user_lines, worktree_path, filepath, info.name, review_mode)
  ...
  if #conflicts > 0 then
    ...
    vim.defer_fn(function() M.show_preview() end, 100)
    vim.notify(string.format("[Vibe] %d conflict(s). Hover to preview. [u]yours [a]AI [b]both [n]none", M.count_remaining_conflicts()), vim.log.levels.INFO)
  else
...
function M.collapse_all_conflicts(bufnr)
  ...
  if #actual_conflicts == 0 then return end
  ...
  for i, conflict in ipairs(actual_conflicts) do
    ...
    local collapse_text = string.format("<<<<<<< CONFLICT: %d lines (yours: %d, AI: %d) >>>>>>>", total_lines, ours_count, theirs_count)
    ...
    vim.fn.sign_place(sign_id, "vibe_collapsed_conflict", "VibeCollapsedConflict", bufnr, { lnum = current_start + 1 })
    vim.api.nvim_buf_set_extmark(bufnr, M.ns, current_start, 0, {
      id = sign_id, end_col = #collapse_text, hl_group = "VibeConflictCollapsed", priority = 200,
    })

    table.insert(new_conflicts, {
      idx = i,
      start_line = current_start, end_line = current_start,
      ours_start = conflict.ours_start, ours_end = conflict.ours_end,
      theirs_start = conflict.theirs_start, theirs_end = conflict.theirs_end,
      hunk = state.conflicts[i] and state.conflicts[i].hunk or nil, resolved = false,
    })
...
function M.get_conflict_at_cursor()
  ...
  for _, conflict in ipairs(state.conflicts) do
    if not conflict.resolved then
      ...
      if line and cursor_line == line then return conflict, conflict.idx end
    end
  end
  return nil, nil
end

function M.is_preview_visible() return M.preview_winnr ~= nil and vim.api.nvim_win_is_valid(M.preview_winnr) end
...
function M.show_preview()
  ...
  if not conflict or conflict.resolved then M.close_preview(); return end
  ...
  if stored_lines then
    for _, line in ipairs(stored_lines) do
      if line:match("^<<<<<<< HEAD") then in_ours, in_theirs = true, false
      elseif line:match("^=======") then in_ours, in_theirs = false, true
      elseif line:match("^>>>>>>> ") then in_ours, in_theirs = false, false
      elseif in_ours then table.insert(ours_lines, line)
      elseif in_theirs then table.insert(theirs_lines, line) end
    end
  end
...
function M.show_floating_preview(bufnr, conflict, idx, ours_lines, theirs_lines)
  ...
  if #ours_lines > 0 then for _, line in ipairs(ours_lines) do table.insert(preview_lines, line) end
  else table.insert(preview_lines, "(empty)") end

  table.insert(preview_lines, "")
  table.insert(preview_lines, "━━━━━━━━━━ AI ━━━━━━━━━━")
  if #theirs_lines > 0 then for _, line in ipairs(theirs_lines) do table.insert(preview_lines, line) end
  else table.insert(preview_lines, "(empty)") end
  ...
  M.preview_winnr = vim.api.nvim_open_win(M.preview_bufnr, false, {
    relative = "win", win = 0, row = row, col = col, width = width, height = height,
    style = "minimal", border = "rounded", title = " Vibe Conflict ", title_pos = "center", zindex = 100,
  })
...
function M.highlight_preview(preview_bufnr, ours_line_count)
  ...
  for i = 1, ours_line_count + 1 do vim.api.nvim_buf_add_highlight(preview_bufnr, ns, "VibePreviewYours", i - 1, 0, -1) end
  for i = ours_line_count + 3, vim.api.nvim_buf_line_count(preview_bufnr) - 3 do vim.api.nvim_buf_add_highlight(preview_bufnr, ns, "VibePreviewTheirs", i - 1, 0, -1) end
  vim.api.nvim_buf_add_highlight(preview_bufnr, ns, "VibePreviewKeymap", vim.api.nvim_buf_line_count(preview_bufnr) - 1, 0, -1)
end
...
function M.setup_preview_keymaps(preview_bufnr, source_bufnr, conflict, idx)
  ...
  vim.keymap.set("n", "u", function() M.close_preview(); M.resolve_conflict("ours") end, opts)
  vim.keymap.set("n", "a", function() M.close_preview(); M.resolve_conflict("theirs") end, opts)
  vim.keymap.set("n", "b", function() M.close_preview(); M.resolve_conflict("both") end, opts)
  vim.keymap.set("n", "n", function() M.close_preview(); M.resolve_conflict("none") end, opts)
  vim.keymap.set("n", "q", M.close_preview, opts)
  vim.keymap.set("n", "<Esc>", M.close_preview, opts)
end

function M.close_preview()
  if M.preview_winnr and vim.api.nvim_win_is_valid(M.preview_winnr) then vim.api.nvim_win_close(M.preview_winnr, true) end
...
function M.resolve_conflict(resolution)
  ...
  local stored_lines = state.conflict_contents and state.conflict_contents[idx]
  if not stored_lines then return false end

  local conflict_line = get_current_line(bufnr, idx)
  if not conflict_line then return false end
  ...
  if resolution == "ours" then replacement_lines = ours_lines
  elseif resolution == "theirs" then replacement_lines = theirs_lines
  elseif resolution == "both" then
    for _, line in ipairs(ours_lines) do table.insert(replacement_lines, line) end
    for _, line in ipairs(theirs_lines) do table.insert(replacement_lines, line) end
  end
  ...
  if conflict.hunk then git.mark_hunk_addressed(state.worktree_path, state.filepath, conflict.hunk, action) end

  local remaining = M.count_remaining_conflicts()
  if remaining == 0 then M.finalize_file()
  else
    M.next_conflict()
    ...
    if not conflict.resolved then
      ...
      if stored_lines then
        ...
        if conflict_line then
          ...
          for _, line in ipairs(stored_lines) do
            if line:match("^=======") then in_theirs = true
            elseif line:match("^>>>>>>> ") then in_theirs = false
            elseif in_theirs then table.insert(theirs_lines, line) end
          end
          ...
          state.resolved_count = state.resolved_count + 1
          if conflict.hunk then git.mark_hunk_addressed(state.worktree_path, state.filepath, conflict.hunk, "accepted") end
        end
      end
    end
  end
...
function M.keep_ours() M.resolve_conflict("ours") end
function M.keep_theirs() M.resolve_conflict("theirs") end
function M.keep_both() M.resolve_conflict("both") end
function M.keep_none() M.resolve_conflict("none") end

function M.count_remaining_conflicts()
  local state = M.buffer_state[vim.api.nvim_get_current_buf()]
  local count = 0
  if state then for _, c in ipairs(state.conflicts) do if not c.resolved then count = count + 1 end end end
  return count
end
...
function M.next_conflict()
  ...
  for _, c in ipairs(state.conflicts) do
    if not c.resolved then
      ...
      if line and line > cursor_line then vim.api.nvim_win_set_cursor(0, { line + 1, 0 }); return end
    end
  end
  for _, c in ipairs(state.conflicts) do
    if not c.resolved then
      local line = get_current_line(bufnr, c.idx)
      if line then vim.api.nvim_win_set_cursor(0, { line + 1, 0 }); return end
    end
  end
end
...
function M.setup_cursor_tracking(bufnr)
  vim.api.nvim_create_autocmd("CursorMoved", {
    ...
    callback = function()
      ...
      if conflict and not conflict.resolved then if not M.is_preview_visible() then M.show_preview() end
      else M.close_preview() end
    end,
  })
...
function M.quit()
  local bufnr = vim.api.nvim_get_current_buf()
  local state = M.buffer_state[bufnr]
  if M.count_remaining_conflicts() > 0 and vim.fn.confirm("Unresolved conflict(s). Quit anyway?", "&Yes\n&No", 2) ~= 1 then return end
  M.close_preview()
  if state then
    vim.cmd("edit!")
  ...
  if state and state.worktree_path then require("vibe.dialog").show(state.worktree_path) end
end
...
function M.finalize_file()
  ...
  if state.auto_merged_regions then
    for _, am in ipairs(state.auto_merged_regions) do
      if am.hunk then git.mark_hunk_addressed(state.worktree_path, state.filepath, am.hunk, "accepted") end
    end
  end
...
function M.is_collapsed_conflict_buffer() return M.buffer_state[vim.api.nvim_get_current_buf()] ~= nil end
