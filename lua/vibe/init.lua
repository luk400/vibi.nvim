...
function M.setup_quit_protection()
  if config.options.quit_protection == false then return end
  ...
  vim.api.nvim_create_autocmd("VimLeavePre", {
    ...
    callback = function()
      ...
      if has_worktrees then
        ...
        local choices = has_unresolved and "&Delete All Worktrees\n&Keep All Worktrees\n&Review Changes\n&Cancel"
        ...
        elseif has_unresolved and choice == 3 then
          vim.schedule(function() session.show_review_list() end)
          return false
...
-- General Public API
function M.toggle(name, cwd) terminal.toggle(name, cwd) end
function M.open(name, cwd) terminal.show(name, cwd) end
function M.close(name) terminal.hide(name) end
function M.kill(name) terminal.kill(name) end
...
function M.list() return session.list() end
function M.show_diff() diff.show_for_current_file() end
function M.clear_diff() diff.clear() end
function M.accept_hunk() diff.accept_hunk() end
function M.reject_hunk() diff.reject_hunk() end
function M.accept_all_in_file() diff.accept_all_in_file() end
function M.reject_all_in_file() diff.reject_all_in_file() end
function M.diff() return diff end
function M.git() return git end
function M.cancel_session() git.cancel_session() end
function M.has_unresolved_changes() return git.has_worktrees_with_changes() end
function M.review() session.show_review_list() end
