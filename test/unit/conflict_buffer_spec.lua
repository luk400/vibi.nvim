...
describe("Conflict Buffer & Merging", function()
  before_each(function()
    for path, _ in pairs(git.worktrees) do git.remove_worktree(path) end
  end)
  ...
  it("auto-merges clean additions smoothly in 'auto' review mode", function()
    ...
    local lines, conflicts, auto_merged = conflict_buffer.insert_conflict_markers(
      user_lines, info.worktree_path, "test.txt", info.name, "auto"
    )
  ...
  it("generates strict git conflict markers for overlapping edits", function()
    ...
    local lines, conflicts, _ = conflict_buffer.insert_conflict_markers(
      user_lines, info.worktree_path, "test.txt", info.name, "auto"
    )
